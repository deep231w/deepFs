...
export default function GoogleButton({ label = 'Continue with Google', href = '/auth/google', className = '' }: Props) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      aria-label={label}
      className={`w-full inline-flex items-center justify-center gap-3 px-4 py-3 border border-gray-200 rounded-lg bg-white text-slate-700 hover:bg-gray-50 shadow-sm transition-transform duration-150 hover:-translate-y-0.5 ${className}`}
    >
